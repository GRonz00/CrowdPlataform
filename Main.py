...
if __name__ == "__main__":
    ...
    for q in mean_not_av:
        ...
        if FINITE_HORIZON:
            plt.figure()
            plt.plot(reps_mean_list[0:N_REPLICATION], marker='o', linestyle='-', color='b', markersize=1, label="mean unavailability time = "+str(mean_not_av[0]))
            plt.plot(reps_mean_list[N_REPLICATION:2*N_REPLICATION], marker='o', linestyle='-', color='g', markersize=1, label="mean unavailability time = "+str(mean_not_av[1]))
            plt.plot(reps_mean_list[2*N_REPLICATION:3*N_REPLICATION], marker='o', linestyle='-', color='r', markersize=1, label="mean unavailability time = "+str(mean_not_av[2]))
            plt.plot(reps_mean_list[3*N_REPLICATION:4*N_REPLICATION], marker='o', linestyle='-', color='c', markersize=1, label="mean unavailability time = "+str(mean_not_av[3]))
            plt.plot(reps_mean_list[4*N_REPLICATION:5*N_REPLICATION], marker='o', linestyle='-', color='m', markersize=1, label="mean unavailability time = "+str(mean_not_av[4]))
            plt.xlabel('Rep Number')
            plt.ylabel('Response Time')
            plt.title('Prob = '+str(p))
            plt.legend()
            plt.grid(True)
            plt.savefig('oriz_fin='+str(pl))
        else:
            plt.figure()
            #if pl>7:
             #   plt.ylim([4.7, 7.5])
            plt.plot(batch_means_list[0:N_BATCH], marker='o', linestyle='-', color='b', markersize=1, label="prob = "+str(p_list[0]))
            plt.plot(batch_means_list[N_BATCH:2*N_BATCH], marker='o', linestyle='-', color='g', markersize=1, label="prob = "+str(p_list[1]))
            plt.plot(batch_means_list[2*N_BATCH:3*N_BATCH], marker='o', linestyle='-', color='r', markersize=1, label="prob = "+str(p_list[2]))
            plt.plot(batch_means_list[3*N_BATCH:4*N_BATCH], marker='o', linestyle='-', color='c', markersize=1, label="prob = "+str(p_list[3]))
            plt.plot(batch_means_list[4*N_BATCH:5*N_BATCH], marker='o', linestyle='-', color='m', markersize=1, label="prob = "+str(p_list[4]))
            plt.xlabel('Batch Number')
            plt.ylabel('Response Time')
            plt.title('Validazione varianza')
            plt.legend()
            plt.grid(True)
            plt.savefig('minuti meanNA= '+str(MEAN_NOT_AVAILABLE_TIME))
    pl+=5
